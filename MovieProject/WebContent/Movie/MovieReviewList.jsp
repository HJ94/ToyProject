...
<body>
	...
	<table class="table table-hover">
		...
		<c:forEach var="moviereviewlist" items="${moviereviewlist}">
			<tr>
				<td>${moviereviewlist.no}</td>
				<td><img
					src="/webapp/imageFolder/${moviereviewlist.image}"
					width=100px; height=100px;></td>
				<td><a
					href="/webapp/MovieController?command=moviereviewdetail&subject=${moviereviewlist.subject}"
					style="color: #000000;">${moviereviewlist.subject}</a></td>
				<td>${moviereviewlist.id}</td>
				<td>${moviereviewlist.indate}</td>
				<td>${moviereviewlist.recommend}</td>
			</tr>
		</c:forEach>
	</table>
